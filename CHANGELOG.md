...
- `downloader.js` — `isCollectionPart` flag passed through to `MoviesDownloader` to support collection-aware path handling
- `downloader.js` — queue (`enqueued`) and reactive edge-triggered `_next_movie` setter: when a download finishes, the next enqueued item starts automatically
